	...
	try {
		...
		if (folders) {
			for (const folder of folders) {
				...
				const children: Sidebar_SubHeadingProps | null =
...
function generateSidebarHeadings(folderPath: string): Sidebar_HeadingProps[] {
	try {
		...
		if (folders) {
			for (const folder of folders) {
				const subFolderPath = path.join(folderPath, folder);
				const children: Sidebar_SubHeadingProps[] =
...
export function generateSidebar() {
	...
	for (const folder of level_1_folders_array ?? []) {
		...
		const children: Sidebar_HeadingProps[] =
			generateSidebarHeadings(folderPath);
