...
import { Button, FieldError, Label, Input as ReactAriaInput, TextField } from "react-aria-components";
...
export function Input({ state = "default", label, isRequired, isReadOnly, className, ...props }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        >
          {active ? <Icon.PasswordVisible className="w-4 h-4" /> : <Icon.PasswordHidden className="w-4 h-4" />}
        </Button>
        <InputBox className={twMerge("pr-12 z-0", className)} {...props} type={active ? "text" : "password"} />
      </div>
