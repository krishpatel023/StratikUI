...
export function Stepper({ children, className, currentStep, onChange }: StepperProps) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: adding currentStep will cause rerender which is not needed
  ...
  return <div className={twMerge("flex flex-col gap-2 relative", className)}>{newChildren}</div>;
}
...
export function StepInternal({ children, className, currentActiveStep, step, totalSteps }: StepInternalProps) {
  const state = useMemo(() => {
  ...
  return (
    <div className={twMerge("group text-foreground flex gap-4 ", className)} data-state={state}>
...
export function StepLine({ className, step, totalSteps, state }: StepLineProps) {
  ...
  return (
    <>
      {showConnectingLine && (
        <div
          className={twMerge("w-1 h-full rounded-full bg-secondary overflow-hidden pointer-events-none ", className)}
        >
          <div
