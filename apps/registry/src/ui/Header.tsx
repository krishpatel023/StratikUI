...
export default function Header() {
  ...
  return (
    <ReactAriaHeader className="h-16 flex items-center justify-between px-8 py-2 shadow shadow-outline">
      <div className="flex items-center justify-center gap-2">
        <Link href="/" className="text-base sm:text-xl font-bold text-foreground hover:underline underline-offset-2">
          StratikUI / Registry
        </Link>
        <Version />
      </div>
      <div className="flex items-center gap-4">
        ...
        <Button variant="ghost" onPress={toggleTheme}>
          {theme ? <Icons.moon className="h-5 w-5" /> : <Icons.sun className="h-5 w-5" />}
        </Button>
      </div>
    </ReactAriaHeader>
  );
...
function Version() {
  return (
    <div className="text-xs font-medium rounded-full border-2 border-accent bg-blue-200/30 dark:bg-blue-800/30 text-accent px-2 py-1 hidden sm:block">
      v {CURRENT_VERSION}
    </div>
  );
}
