...
export const Data = {
    name: "Default Dropdown Menu",
    tags: [],
    description: "Dropdowns are versatile UI components that provide a list of selectable options within a compact and accessible interface. This accessible dropdown menu provides features including keyboard navigation, single or multiple item selection, and various trigger interactions. It supports arrow key navigation, typeahead, auto scrolling, and disabled items. Menus can be opened by mouse, touch, or keyboard interactions, focusing the first or last item automatically. ",
...
<ImportsWrapper>
import { DropdownImplementation } from "@/packages/primitives/dropdown/Dropdown_1";
