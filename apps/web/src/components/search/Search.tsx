...
import type { SearchIndividualProps } from "@/data/Search";
...
export default function CommandPalette({
  ...
  return (
    <>
      <CommandPalettePrimitive isOpen={searchbar} onOpenChange={(val) => setSearchbar(val)}>
        <CommandPaletteSearchBar placeholder="Search for something" onChange={(e) => search(e.target.value)} />
        <CommandPaletteDivider />
        <CommandPaletteMenu isLoading={isLoading} style={{ "--sidebar-width": "5px" } as CSSProperties}>
...
function SearchItems({
  ...
  return (
    <CommandPaletteItem className="flex items-center justify-start gap-6 my-2 h-16" link={link} onPress={onPress}>
      {category === "components" ? (
      ...
      <div className="flex flex-col items-start">
        ...
        <span className="hidden sm:block text-sm capitalize text-secondary-foreground">{category}</span>
      </div>
    </CommandPaletteItem>
  );
}
function DefaultItems({ onPress }: { onPress: () => void }) {
  return (
    <CommandPaletteGroup heading="Connect">
      ...
      <CommandPaletteItem className="flex items-center justify-start gap-6" link={Links.stratikui.github} newTab>
        <Icons.github className="w-4 h-4" />
        <span>View Repository</span>
      </CommandPaletteItem>
      <CommandPaletteItem className="flex items-center justify-start gap-6" link={Links.stratikui.newIssue} newTab>
...
const Icons = {
  components: (props: IconProps) => (
    <svg height="200" width="200" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" {...props}>
      ...
      <path d="M2 2h20v20H2V2Zm2 2v7h7V4H4Zm9 0v16h7V4h-7Zm-2 16v-7H4v7h7Z" fill="currentColor" />
    </svg>
  ),
  primitive: (props: IconProps) => (
    <svg height="200" width="200" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" {...props}>
      <title>Primitive Icon</title>
  ...
  github: (props: IconProps) => (
    <svg height="200" width="200" viewBox="0 0 432 416" xmlns="http://www.w3.org/2000/svg" {...props}>
