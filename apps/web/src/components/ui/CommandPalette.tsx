...
import { type Key, KeyListener, KeyListenerDisplay } from "@/components/ui/KeyListener";
...
export function CommandPaletteItem({ children, className, link, newTab, onPress, ...props }: CommandPaletteItemProps) {
  ...
  return (
    ...
    >
      ...
      {link && (
        <Link className="hidden" href={link} ref={ref} {...(newTab ? { target: "_blank", rel: "noreferrer" } : {})} />
      )}
    </Button>
  );
}
...
export function CommandPalette({ isOpen, onOpenChange, children, className }: CommandPaletteProps) {
  return (
    <ModalTrigger isOpen={isOpen} onOpenChange={onOpenChange}>
      <Modal
        className={twMerge("max-h-[25rem] w-80 md:w-[30rem] p-0 bg-white dark:bg-neutral-950", className as string)}
        isDismissable
      >
        {children}
      </Modal>
...
export function CommandPaletteMenu({ children, isLoading, isEmpty, className, ...props }: CommandPaletteMenuProps) {
  ...
  useEffect(() => {
    if (menuRef.current) {
      const menuItemsFound = menuRef.current.querySelectorAll("[data-cmd-items]") as NodeListOf<HTMLElement>;
  ...
  return (
    <Group
      className={twMerge("w-full max-h-80 z-10 py-2 px-2 overflow-y-auto scrollbar-x scrollbar-y", className as string)}
      ref={menuRef}
      {...props}
    >
...
export function CommandPaletteSearchBar({ className, ...props }: CommandPaletteSearchBarProps) {
  function handleKeyDown(event: any) {
    if (event.key === "ArrowDown") {
      const menuItemsFound = document.querySelectorAll("[data-cmd-items]") as NodeListOf<HTMLElement>;
      const arr = Array.from(menuItemsFound);
...
export function CommandPaletteDivider({ className }: { className?: string }) {
  return <div className={twMerge("min-w-full border-[0.5px] border-outline-secondary", className)} />;
}
  ...
  return (
    <div className="w-full h-full flex items-center justify-end px-4 pb-2 gap-2">
      {listeners.map((item, index) => (
        <div key={index} className="flex gap-2 items-center text-neutral-700 dark:text-neutral-200 text-sm">
          <KeyListenerDisplay keys={item.key} key={index} className="text-xs py-0.5" />
          <span>{item.name}</span>
        </div>
      ))}
    </div>
