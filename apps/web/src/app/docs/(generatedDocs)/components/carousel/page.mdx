import React, { Suspense } from 'react';
import { Implementation, Preview, Header, TechnologyUsed, Wrapper, Skeleton, PARTITION } from "@/components/mdx/MDXServerImports"
import { CodeBlock, ResizableDisplay } from "@/components/mdx/MDXClientImports"


{/*
// ------------------------------------------------------------------------------------
//	The text below is generated from apps/web/src/content/docs/components/carousel/Carousel_01.mdx
// ------------------------------------------------------------------------------------
*/}


<Wrapper>

# Logo Carousel
### This is an infinite logo Carousel developed using only tailwind. It has a direction attribute that controls the direction of the animation. The default direction is left.





export const CarouselImplementation_components_carousel_carousel_01 = React.lazy(() =>
	import('@/packages/components/logo_carousel/Carousel_1')
		.then((mod) => ({
			default: mod.CarouselImplementation
	}))
);


<Header />
<Preview> 

<Suspense fallback={<Skeleton className="h-[800px] rounded-lg" />}>
<ResizableDisplay>
<CarouselImplementation_components_carousel_carousel_01 />
</ResizableDisplay>
</Suspense>

</Preview>

<Implementation>


<TechnologyUsed technologies={[ "tailwind-css" , "twMerge"]}/>
<CodeBlock>
 
```tsx|Carousel
import Logo from "@/packages/helper/Logo";

export function Carousel({
  direction = "left",
}: {
  direction?: "left" | "right";
}) {
  const animateInDirection =
    direction === "right"
      ? "animate-infinite-scroll-right"
      : "animate-infinite-scroll-left";
  return (
    <>
      <div className="max-w-[80rem] h-28 inline-flex flex-nowrap overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-128px),transparent_100%)]">
        <ul
          className={`flex items-center justify-center md:justify-start [&_li]:mx-8 @md:[&_li]:mx-16  ${animateInDirection}`}
        >
          {Logo.map((item, i) => (
            <li key={i} className="p-4 hover:bg-secondary rounded-md">
              <item.logo className="w-10 h-10 text-primary-foreground" />
            </li>
          ))}
        </ul>
        <ul
          className={`flex items-center justify-center md:justify-start [&_li]:mx-8 @md:[&_li]:mx-16   ${animateInDirection}`}
        >
          {Logo.map((item, i) => (
            <li key={i} className="p-4 hover:bg-secondary rounded-md">
              <item.logo className="w-10 h-10 text-primary-foreground" />
            </li>
          ))}
        </ul>
      </div>
    </>
  );
}
```
```jsx|Carousel
import Logo from "@/packages/helper/Logo";

export function Carousel({
  direction = "left",
}) {
  const animateInDirection =
    direction === "right"
      ? "animate-infinite-scroll-right"
      : "animate-infinite-scroll-left";
  return (
    <>
      <div className="max-w-[80rem] h-28 inline-flex flex-nowrap overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-128px),transparent_100%)]">
        <ul
          className={`flex items-center justify-center md:justify-start [&_li]:mx-8 @md:[&_li]:mx-16  ${animateInDirection}`}
        >
          {Logo.map((item, i) => (
            <li key={i} className="p-4 hover:bg-secondary rounded-md">
              <item.logo className="w-10 h-10 text-primary-foreground" />
            </li>
          ))}
        </ul>
        <ul
          className={`flex items-center justify-center md:justify-start [&_li]:mx-8 @md:[&_li]:mx-16   ${animateInDirection}`}
        >
          {Logo.map((item, i) => (
            <li key={i} className="p-4 hover:bg-secondary rounded-md">
              <item.logo className="w-10 h-10 text-primary-foreground" />
            </li>
          ))}
        </ul>
      </div>
    </>
  );
}
```

```ts|tailwind.config
type { Config } from "tailwindcss";
const config: Config = {
  //...
  theme: {
    extend: {
      //...
      animation: {
        "infinite-scroll-left": "infinite-scroll-left 50s linear infinite",
        "infinite-scroll-right": "infinite-scroll-right 50s linear infinite",
      },
      keyframes: {
        "infinite-scroll-left": {
          from: { transform: "translateX(0)" },
          to: { transform: "translateX(-100%)" },
        },
        "infinite-scroll-right": {
          from: { transform: "translateX(-100%)" },
          to: { transform: "translateX(0%)" },
        },
      },
    },
  },
};
export default config;
```

```js|tailwind.config
const config = {
  //...
  theme: {
    extend: {
      //...
      animation: {
        "infinite-scroll-left": "infinite-scroll-left 50s linear infinite",
        "infinite-scroll-right": "infinite-scroll-right 50s linear infinite",
      },
      keyframes: {
        "infinite-scroll-left": {
          from: { transform: "translateX(0)" },
          to: { transform: "translateX(-100%)" },
        },
        "infinite-scroll-right": {
          from: { transform: "translateX(-100%)" },
          to: { transform: "translateX(0%)" },
        },
      },
    },
  },
};
export default config;
```

</CodeBlock>


</Implementation>
</Wrapper>
<PARTITION />
