import React, { Suspense } from 'react';
import { Implementation, Preview, Header, Display, TechnologyUsed, Wrapper, Skeleton, PARTITION } from "@/components/mdx/MDXServerImports"
import { CodeBlock } from "@/components/mdx/MDXClientImports"


{/*
// ------------------------------------------------------------------------------------
//	The text below is generated from apps/web/src/content/docs/primitives/checkbox/Checkbox_01.mdx
// ------------------------------------------------------------------------------------
*/}


<Wrapper>

# Default Checkbox
### A checkbox allows a user to select multiple items from a list of individual items, or to mark one individual item as selected.





export const CheckboxImplementation_primitives_checkbox_checkbox_01 = React.lazy(() =>
	import('@/packages/primitives/checkbox/CheckBox_1')
		.then((mod) => ({
			default: mod.CheckboxImplementation
	}))
);


<Header />

<Preview> 
<Suspense fallback={<Skeleton className="h-60 rounded-lg" />}>
<Display>
  <CheckboxImplementation_primitives_checkbox_checkbox_01 /> 
</Display>
</Suspense>
</Preview>

<Implementation>
<TechnologyUsed technologies={[ "tailwind-css" , "react-aria", "twMerge" ]}/>

<CodeBlock>
 
```tsx|Checkbox
"use client";

import {
  CheckboxProps,
  Checkbox as ReactAriaCheckbox,
} from "react-aria-components";
import { twMerge } from "tailwind-merge";

export const Checkbox = ({ children, className, ...props }: CheckboxProps) => {
  return (
    <ReactAriaCheckbox
      className={twMerge(
        "group text-foreground hover:cursor-pointer flex gap-2 ",
        className as string
      )}
      {...props}
    >
      <>
        <div className="mt-[0.125rem] min-w-5 max-w-5 min-h-5 max-h-5 flex items-start justify-center border border-outline rounded group-data-[selected=true]:bg-accent group-data-[focused=true]:ring-2 group-data-[focused=true]:ring-accent transition-all duration-200">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 16 16"
            aria-hidden="true"
            className="w-full h-full fill-none group-data-[selected=true]:fill-accent-foreground"
          >
            <path
              fillRule="evenodd"
              d="M13.488 3.43a.75.75 0 0 1 .081 1.058l-6 7a.75.75 0 0 1-1.1.042l-3.5-3.5A.75.75 0 0 1 4.03 6.97l2.928 2.927l5.473-6.385a.75.75 0 0 1 1.057-.081Z"
              clipRule="evenodd"
            />
          </svg>
        </div>
        {children}
      </>
    </ReactAriaCheckbox>
  );
};
```
```jsx|Checkbox
"use client";

import {
  Checkbox as ReactAriaCheckbox,
} from "react-aria-components";
import { twMerge } from "tailwind-merge";

export const Checkbox = ({ children, className, ...props }) => {
  return (
    <ReactAriaCheckbox
      className={twMerge(
        "group text-foreground hover:cursor-pointer flex gap-2 ",
        className
      )}
      {...props}
    >
      <>
        <div className="mt-[0.125rem] min-w-5 max-w-5 min-h-5 max-h-5 flex items-start justify-center border border-outline rounded group-data-[selected=true]:bg-accent group-data-[focused=true]:ring-2 group-data-[focused=true]:ring-accent transition-all duration-200">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 16 16"
            aria-hidden="true"
            className="w-full h-full fill-none group-data-[selected=true]:fill-accent-foreground"
          >
            <path
              fillRule="evenodd"
              d="M13.488 3.43a.75.75 0 0 1 .081 1.058l-6 7a.75.75 0 0 1-1.1.042l-3.5-3.5A.75.75 0 0 1 4.03 6.97l2.928 2.927l5.473-6.385a.75.75 0 0 1 1.057-.081Z"
              clipRule="evenodd"
            />
          </svg>
        </div>
        {children}
      </>
    </ReactAriaCheckbox>
  );
};
```
```tsx|Implementation
export const CheckboxImplementation = () => {
  return <Checkbox>Checkbox</Checkbox>;
};
```
```jsx|Implementation
export const CheckboxImplementation = () => {
  return <Checkbox>Checkbox</Checkbox>;
};
```

</CodeBlock>



Checkout the default documentation of [React Aria Checkbox](https://react-spectrum.adobe.com/react-aria/Checkbox.html)

</Implementation>
</Wrapper>
<PARTITION />
