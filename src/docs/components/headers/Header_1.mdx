...
---
title : Responsive Default Header Section
tags : 
version : "0.1.2"
display : true
---

# Responsive Default Header Section
## This is a default header section with continuous dropdown. Once a link is hovered and if it shifts the navbar to the next link the dropdown will not close, instead it will just shift. It also has click outside hook to close the header when clicked outside.


<HeaderDefault id={convertToDashed("Responsive Default Header Section")}/> 
    <ResizableDisplay id={convertToDashed("Responsive Default Header Section")}>
        <div className="min-h-[40rem] w-full">
            <Header/>
        </div>
    </ResizableDisplay>

<Implementation id={convertToDashed("Responsive Default Header Section")}>
<CodeBlock> 
```tsx|Header
export const Header = () => {
