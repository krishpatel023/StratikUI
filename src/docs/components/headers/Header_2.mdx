...
# Header Section with animated continuous dropdown
## This is a default header section with continuous dropdown. It acts same as the previous one but has framer motion animations added to it. It also has click outside hook to close the header when clicked outside.


<HeaderDefault id={convertToDashed("Header Section with animated continuous dropdown")}/> 
    <ResizableDisplay id={convertToDashed("Header Section with animated continuous dropdown")}>
        <div className="min-h-[40rem] w-full">
            <Header/>
        </div>
    </ResizableDisplay>

<Implementation id={convertToDashed("Header Section with animated continuous dropdown")}>
<CodeBlock> 
```tsx|Header
export const Header = () => {
